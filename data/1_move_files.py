...
def get_train_test_lists(version='01'):
    ...
    test_file = '/home/wpc/five-video-classification-methods-master/data/ucfTrainTestlist/testlist' + version + '.txt'
    train_file = '/home/wpc/five-video-classification-methods-master/data/ucfTrainTestlist/trainlist' + version + '.txt'

    # Build the test list.
    with open(test_file) as fin:
        test_list = [row.strip() for row in list(fin)]
        test_list = ['UCF101/'+row for row in test_list]

    # Build the train list. Extra step to remove the class index.
    with open(train_file) as fin:
        train_list = [row.strip() for row in list(fin)]
        train_list = [row.split(' ')[0] for row in train_list]
        train_list = ['UCF101/'+row for row in train_list]
